...
End Function
    ...
    Public Shared Function CheckUserPermissions(ByVal sTable As String, ByVal sAction As String) As Boolean
        ...
        Dim sql As String
        Dim con As New SqlConnection(ConfigurationManager.ConnectionStrings("SA_SystemConnectionString").ToString)
        sql = "select permission from counter.user_permissions where userid='" + sUserName + "' and tablename='" + sTable + "' and action='" + sAction + "'"
...
Public Function Login(ByVal txtUser As String, ByVal txtPassword As String) As UserClass
	...
	If sLoginFrom = "DB" Then
		...
		Dim sPwdType As String = ConfigurationManager.AppSettings("FieldUserPwdType")
            ...
            Dim sSQL As String = "select [" + sLogin + "],[" + sPwd + "],[firstpage]"
...
End Sub
    ...
    Public Shared Sub login_page(ByVal User_name As String, ByVal Page As String, ByVal Login_Type As String)
        ...
        Dim Dr As DataRow
        Dim StrCon As New SqlConnection("Server=172.30.203.155; uid=sa;pwd=1234; database= SA_System")
        Dim Adpt As New SqlDataAdapter("SELECT * FROM counter.[VSPP_HIS]", StrCon)
